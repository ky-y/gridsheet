    ...
    const handleKeyDown = useCallback(
        (e: KeyboardEvent<HTMLDivElement>) => {
            ...
            if (
                ...
                const sMinRow = Math.min(
                    selection.start.row,
                    selection.end.row,
                );
                const sMaxRow = Math.max(
                    selection.start.row,
                    selection.end.row,
                );
                const sMinCol = Math.min(
                    selection.start.col,
                    selection.end.col,
                );
                const sMaxCol = Math.max(
                    selection.start.col,
                    selection.end.col,
                );
            ...
            setSelection((prev) => {
                ...
                if (e.shiftKey && (e.ctrlKey || e.metaKey)) {
                    ...
                    if (dir.row !== 0)
                        newEnd.row = dir.row < 0 ? minRow : maxRow;
                    if (dir.col !== 0)
                        newEnd.col = dir.col < 0 ? minCol : maxCol;
                ...
                if ((e.ctrlKey || e.metaKey) && !e.shiftKey) {
                    ...
                    if (dir.row !== 0)
                        newPos.row = dir.row < 0 ? minRow : maxRow;
                    if (dir.col !== 0)
                        newPos.col = dir.col < 0 ? minCol : maxCol;
